...
We solved three problems from the course problem set. *Robert Hood* (geometry) finds the diameter of a point set using Andrew's monotone chain convex hull followed by rotating calipers in $O(n log n)$. *Cookie Selection* (algorithms on arrays) maintains a dynamic multiset under insertions and median queries using a Fenwick tree with coordinate compression and binary descent in $O(n log n)$. *Free Real Estate* (randomized algorithms) applies a randomized approach to its core subproblem.
