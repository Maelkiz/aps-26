...
We solved three problems, each of which corresponds to one of the categories presented in the course. *Robert Hood* (Geometric Algorithms) finds the diameter of a point set using #highlight[Andrew's monotone chain convex hull followed by rotating calipers in $O(n log n)$]. *Cookie Selection* (Algorithms on Arrays) #highlight[maintains a dynamic multiset under insertions and median queries using a Fenwick tree with coordinate compression and binary descent in $O(n log n)$]. *Jagged Skyline* (randomized algorithms) #highlight[uses Monte Carlo column sampling with binary search to find the tallest building in $O(k log h)$ queries expected, where $k$ is the number of promising columns.]
