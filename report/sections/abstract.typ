...
#highlight[We solved three problems from the course problem set]. *Robert Hood* (geometric algorithms) finds the diameter of a point set using #highlight[Andrew's monotone chain convex hull followed by rotating calipers in $O(n log n)$]. *Cookie Selection* (interval queries) #highlight[maintains a dynamic multiset under insertions and median queries using a Fenwick tree with coordinate compression and binary descent in $O(n log n)$]. *Jagged Skyline* (randomized algorithms) applies a #highlight[randomized approach to its core subproblem.]
